...
<main>
	...
	<div class="topCard cycle-slideshow  " 
		...
		>
	<c:forEach var="images" items="${detail }">
	...
	<div class="col-5 text-center" style="margin-left: 29%;">
		<c:forEach var="info" items="${detail }" begin="0" end="0">
		<hr>
		<span>
		...
		<br><span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ${info.location}</span>   
		<br>
		<hr>
		<p class="fw-bold text-dark h5">${info.title }</p>
		<p><fmt:formatDate var="regDate" value="${info.regDate }" pattern="yyyy년 MM월 dd일 HH시 mm분" />${regDate } </p>
		<p class="fw-bold text-dark h5"> <fmt:formatNumber var="price" value="${info.price }" pattern="#,###"/>￦ ${price }원</p><br>
		<textarea class="form-control" rows="15"  >${info.content }</textarea> <br>
		<p class="fw-bold text-dark"><span>조회수 : ${info.hitCnt } &nbsp;</span><span>추천 : ${info.likeCnt } &nbsp;</span> </p>
		</c:forEach>
	</div>
	<br>
