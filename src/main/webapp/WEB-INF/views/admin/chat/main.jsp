		...
		<div class="row">
			<div class="col-2">
				<select id="searchType" name="searchType" class="form-control">
					...
					<option value="userId" <c:if test="${param.searchType eq 'userId' }">selected='selected'</c:if>>사용자 아이디</option>
					<option value="notReadCnt" <c:if test="${param.searchType eq 'notReadCnt' }">selected='selected'</c:if>>안읽은 메세지(이상)</option>
				</select>
			</div>
			<div class="col-3">
			...
			<div class="col-2">
				<button onclick="javascript: self.location='/admin/chat/main'" type="button" class="form-control btn btn-light">검색 초기화</button>
			</div>
		</div>
	</form>
</section>
	...
	<table class="table table-hover table-bordered fw-bold">
		...
		<c:forEach var="list" items="${list }" >	
			<tr>	
				<td>${list.rnum }</td>
				<td>${list.userId }</td>
				<td>${list.notReadCnt } 건</td>
				<td><fmt:formatDate value="${list.createDate }" pattern="yyyy-MM-dd HH:mm:ss"/> </td>
				<td><button type="button" class="btn btn-light adminChatBtn form-control" value="${list.targetUserNo }">답변하기</button></td>
			</tr>
		</c:forEach>
...
$(document).ready(function(){
    ...
    $("#searchBtn").on("click",function(){
    	self.location="/admin/chat/main?keyword="+$("#keyword").val()+"&searchType="+$("#searchType").val()+"&startDate="+$("#startDate").val()+"&endDate="+$("#endDate").val()+"&pagePerNum="+$("#pagePerNum").val()
    });
    ...
    $("#excelDown").on("click",function(){
    	$("#excelForm").attr("action","/admin/chat/excelDown")
		$("#excelForm").submit();
    ...
    //채팅 답변 페이지로 이동
    $(".adminChatBtn").on("click",function(){
    	location.href="/admin/chat/detail?targetUserNo="+$(this).val()
    })
