		...
		<div class="row">
			<div class="col-2">
				<select id="searchType" name="searchType" class="form-control">
					...
					<option value="S"  <c:if test="${param.searchType eq 'S' }">selected='selected'</c:if>>답변대기</option>
					<option value="Y"  <c:if test="${param.searchType eq 'Y' }">selected='selected'</c:if>>답변완료</option>
				</select>
			</div>
			<div class="col-3">
			...
			<div class="col-2">
				<button onclick="javascript: self.location='/admin/question/main'" type="button" class="form-control btn btn-light">검색 초기화</button>
			</div>
		</div>
	</form>
</section>
...
<main class="container-fluid">
	...
	<table class="table table-hover table-bordered fw-bold">
		...
		<tr>
			...
			<td ><a class="text-dark" href="/admin/question/detail?inquiryNo=${list.inquiryNo}">${list.title }</a></td>
			<td>${list.writer}</td>
			<td>${list.hitCnt}</td>
			<c:choose>
				<c:when test="${list.status eq 'S' }"> 
					<td class="text-warning">답변 대기중</td>
				</c:when>
...
$(document).ready(function(){
    ...
    $("#searchBtn").on("click",function(){
    	self.location="/admin/question/main?keyword="+$("#keyword").val()+"&searchType="+$("#searchType").val()+"&startDate="+$("#startDate").val()+"&endDate="+$("#endDate").val()+"&pagePerNum="+$("#pagePerNum").val()
    });
  	//pagePerNum
    ...
    $("#excelDown").on("click",function(){
    	$("#excelForm").attr("action","/admin/question/excelDown")
		$("#excelForm").submit()
