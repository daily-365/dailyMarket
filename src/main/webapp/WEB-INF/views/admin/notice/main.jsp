		...
		<div class="row">
			<div class="col-2">
				<select id="searchType" name="searchType" class="form-control">
					...
					<option value="title" <c:if test="${param.searchType eq 'writer' }">selected='selected'</c:if>>제목</option>
					<option value="content"  <c:if test="${param.searchType eq 'loc' }">selected='selected'</c:if>>내용</option>
				</select>
			</div>
			<div class="col-3">
			...
			<div class="col-2">
				<button onclick="javascript: self.location='/admin/notice/main'" type="button" class="form-control btn btn-outline-secondary">검색 초기화</button>
			</div>
		</div>
	</form>
</section>
...
<main class="container-fluid">
	<br>
	<h4 class="text-center">공지사항 등록 현황</h4>
	<br>
	<table class="table table-hover table-bordered fw-bold container">
		<tr>
			<th>#</th>
			<th>번호</th>
			<th>제목</th>
		...
		<tr>
			<td>${list.rnum }</td>
			<td>${list.noticeNo }</td>
			<td><a  class="text-dark"  href="/admin/notice/detail?noticeNo=${list.noticeNo}">${list.title}</a></td>
...
<br><br>
	<div class="text-center">
		<a class="btn btn-secondary" href="${pageMaker.makeSearch(1) }">&laquo;&laquo;</a>
		<c:if test="${pageMaker.prev }">
			<a class="btn btn-outline-secondary" href="${pageMaker.makeSearch(pageMaker.startPage-1) }">&laquo;</a>
		</c:if>
		<c:forEach var="idx" begin="${pageMaker.startPage }" end="${pageMaker.endPage +1}"  >
			<a class="btn btn-outline-secondary <c:if test="${param.page eq idx}">active</c:if>"  href="${pageMaker.makeSearch(idx) }">${idx}</a>
		</c:forEach>
		<c:if test="${pageMaker.next && pageMaker.endPage>0}">
			<a class="btn btn-outline-secondary" href="${pageMaker.makeSearch(pageMaker.endPage+1)}">&raquo;</a>
		</c:if>
		<a class="btn btn-secondary" href="${pageMaker.makeSearch(pageMaker.maxPage+1)}">&raquo;&raquo;</a>
	</div>
</main>
<br><br>
</body>
...
$(document).ready(function(){
    ...
    $("#searchBtn").on("click",function(){
    	self.location="/admin/notice/main?keyword="+$("#keyword").val()+"&searchType="+$("#searchType").val()+"&startDate="+$("#startDate").val()+"&endDate="+$("#endDate").val()+"&pagePerNum="+$("#pagePerNum").val()
    });
  	//pagePerNum
    ...
    $("#excelDown").on("click",function(){
    	$("#excelForm").attr("action","/admin/notice/excelDown")
		$("#excelForm").submit();
